...
class ERA5_parameters_preproccess_PCA(object):
    ...
    def __call__(self):
        ...
        Temperature_Troppause_1d = np.empty(64800 * 132 * 28)
        ...
        Temperature_Troppause_1d_N = stats.zscore(
            Temperature_Troppause_1d
        )  # T
    ...
    def read_parameters_from_netcdf(self, FILE_NAME_ERA):
        ...
        T_Troppause = self.Tropopause_Temperature(T_all)
    ...
    def Tropopause_Temperature(self, T_all):
        # find the lowmost temperature in the 100-300 hPa layer
        # output is a 3D array (time x lat x lon)
        # just like the input array
        T_troppause = np.empty(
            (T_all.shape[0], T_all.shape[2], T_all.shape[3])
        )

        # Find the minimum value along the first axis of T_all
        min_vals = np.nanmin(T_all, axis=1)

        # Assign the minimum values to T_troppause
        T_troppause[:] = min_vals[:]
    ...
    def preproccess_parameters(
        ...
        T_Troppause = np.delete(T_Troppause, 0, axis=1)
        ...
        T_Troppause_1 = np.array(
            np.flipud(T_Troppause[day, :, :])
        ).reshape(64800)
    ...
    def unstability_calculator(
        ...
        dewpoint300 = np.array(
            mpcalc.dewpoint_from_relative_humidity(
                T300_1 * units.kelvin,
                RH300_1 / 100 * units.dimensionless,
            )
        )
        dewpoint250 = np.array(
            mpcalc.dewpoint_from_relative_humidity(
                T250_1 * units.kelvin,
                RH250_1 / 100 * units.dimensionless,
            )
        )
        sitaE300 = np.array(
...
def save_PCA_data_as_netcdf(PC_all):
    ...
    ds.to_netcdf(
        "/RAID01/data/PC_data/2010_2020_4_parameters_300hPa_PC1_Troppause_temperature.nc"
    )
