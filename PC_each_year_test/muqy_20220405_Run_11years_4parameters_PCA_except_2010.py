...
class ERA5_parameters_preproccess_PCA(object):
    ...
    def preproccess_parameters(
        ...
        RH300_1[RH300_1 <= 0] = 0.001
        RH250_1[RH250_1 <= 0] = 0.001
    ...
    def Principle_Component_Analysis(
        ...
        PC_all_each_year = np.empty((10 * 21772800, 4))
        PC_all = np.empty((10 * 21772800))

        PC_all_each_year[:, 0] = RelativeH_1d_N.reshape(
            10 * 21772800
        )
        PC_all_each_year[:, 1] = Temperature_1d_N.reshape(
            10 * 21772800
        )
        PC_all_each_year[:, 2] = Wvelocity_1d_N.reshape(
            10 * 21772800
        )
        PC_all_each_year[:, 3] = Stability_1d_N.reshape(
            10 * 21772800
        )
        ...
        # pca.fit(PC_all)
        PC_all[:] = pca.fit_transform(
            PC_all_each_year[:, :]
        ).squeeze()

        PC_all[:] = stats.zscore(PC_all[:])

        PC_all = PC_all.reshape(10, 12, 28, 180, 360)
...
def save_PCA_data_as_netcdf(PC_all):
    ...
    ds = xr.Dataset(
        ...
        coords={
            ...
            "month": ("months", np.linspace(0, 119, 120)),
    ...
    ds.to_netcdf(
        "/RAID01/data/PC_data/2010_2020_4_parameters_300hPa_PC1_except_2010.nc"
    )
