...
class ERA5_parameters_preproccess_PCA(object):
    ...
    def __init__(
        ...
        self.ERA_FILE_PATH = "/RAID01/data/ERA5_data/"
        self.ERA_FILE_NAME = "ERA5_daily_"

    def __call__(self):
        # Make auxiliary arrays for atmospheric variables
        RelativeH_1d = np.empty(64800 * 372 * 28)
        Temperature_1d = np.empty(64800 * 372 * 28)
        Wvelocity_1d = np.empty(64800 * 372 * 28)
        Stability_1d = np.empty(64800 * 372 * 28)
    ...
    def read_parameters_from_netcdf(self, FILE_NAME_ERA):
        ...
        return (
            ...
            T.astype(np.float32),
            W.astype(np.float32),
            T250.astype(np.float32),
            T300.astype(np.float32),
            RH300.astype(np.float32),
            RH250.astype(np.float32),
            W300.astype(np.float32),
            Z300.astype(np.float32),
            Z250.astype(np.float32),
        )
    ...
    def Principle_Component_Analysis(
        ...
        """
        PC_all = np.empty((64800 * 372 * 28, 4))
        PC1 = np.empty((64800 * 372 * 28, 1))
        ...
        PC1 = PC1.reshape(64800 * 372 * 28)
        PC1 = PC1.reshape(372, 28, 180, 360)
...
def save_PCA_data_as_netcdf(PC_all):
    ...
    PC_all = PC_all.reshape(372, 28, 180, 360)  # PCA
