...
# # sns.set()
# # ax1 = sns.kdeplot(A_N30 ,legend=True,label="2010")
# # ax2 = sns.kdeplot(A_N40 ,legend=True,label="2011")
# ax3 = sns.kdeplot(A_N50 ,legend=True,label="2012",ax=axes[0])
# # ax4 = sns.kdeplot(A_N60 ,legend=True,label="2013")
# ax5 = sns.kdeplot(A_N70 ,legend=True,label="2014",ax=axes[1])
# # ax6 = sns.kdeplot(A_N80 ,legend=True,label="2015")
# ax7 = sns.kdeplot(A_N90 ,legend=True,label="2016",ax=axes[2])
# # ax8 = sns.kdeplot(A_N100 ,legend=True,label="2017")
# ax9 = sns.kdeplot(A_N101 ,legend=True,label="2018",ax=axes[3])
# ax10 = sns.kdeplot(A_N102 ,legend=True,label="2019",ax=axes[4])
# # ax11 = sns.kdeplot(A_N103 ,legend=True,label="2020")

# sns.distplot(A_N1,hist=False,fit=norm)

# np.sum(A_N100[A_N100>10])

# plt.plot(np.sort(A_N1),stats.norm.pdf(np.sort(A_N1)),label="all")
# # plt.plot(np.sort(A_N50),stats.norm.pdf(np.sort(A_N50)),label="2012")
# # plt.plot(np.sort(A_N60),stats.norm.pdf(np.sort(A_N50)),label="2013")
# # plt.plot(np.sort(A_N70),stats.norm.pdf(np.sort(A_N50)),label="2014")
# # plt.plot(np.sort(A_N80),stats.norm.pdf(np.sort(A_N50)),label="2015")
# # plt.plot(np.sort(A_N90),stats.norm.pdf(np.sort(A_N50)),label="2016")
# plt.plot(np.sort(A_N100),stats.norm.pdf(np.sort(A_N50)),label="2017")
# plt.plot(np.sort(A_N101),stats.norm.pdf(np.sort(A_N50)),label="2018")
# plt.plot(np.sort(A_N102),stats.norm.pdf(np.sort(A_N50)),label="2019")
# plt.plot(np.sort(A_N103),stats.norm.pdf(np.sort(A_N50)),label="2020")
...
#         if A_N90[:,i,j][np.where((A_N19[:,i,j]>=0.95)&(A_N19[:,i,j]<3.95))].shape == 0:
#             A_NNN[i,j] = np.nan
#         else :
#             A_NNN[i,j] = np.nanmean(A_N90[:,i,j][np.where((A_N19[:,i,j]>=0.95)&(A_N19[:,i,j]<3.95))])

# for i in range(0,180):
#     for j in range(0,360):
#         if A_N102[:,i,j][np.where((A_N112[:,i,j]>=0.45)&(A_N112[:,i,j]<0.95))].shape == 0:
#             A_NNN1[i,j] = np.nan
#         else :
#             A_NNN1[i,j] = np.nanmean(A_N102[:,i,j][np.where((A_N112[:,i,j]>=0.45)&(A_N112[:,i,j]<0.95))])
...
a = map.pcolormesh(
    lon, lat, DDD, norm=MidpointNormalize(midpoint=0), cmap=cmap
)
fig.colorbar(a, shrink=0.6)
ax.set_title("2019 minus 2018 Cldtau(0.95>EOF>=0.45)", size=16)
