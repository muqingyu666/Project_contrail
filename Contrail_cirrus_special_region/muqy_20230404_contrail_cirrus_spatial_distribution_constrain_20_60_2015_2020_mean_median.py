...
def compute_median_HCF_abnormal_masked(
    AOD_range, IWP_range, data, mask
):
    """
    Compute the mean of the masked data.
        ...
        for year in range(2015, 2021):
            ...
            for AOD_idx, AOD_slice in enumerate(AOD_slices):
                for IWP_idx, IWP_slice in enumerate(IWP_slices):
                    ...
                    masked_data_mean = (
                        compute_mean_HCF_abnormal_masked(
                            AOD_slice, IWP_slice, data_year, mask
                        )
                    )
                    masked_data_median = (
                        compute_median_HCF_abnormal_masked(
                            AOD_slice, IWP_slice, data_year, mask
                        )
                    )
    ...
    for col_idx, AOD_case in enumerate(AOD_cases):
        ...
        for row_idx, IWP_case in enumerate(IWP_cases):
            ...
            for region in region_names:
                ...
                # mean by all spatial points
                data_to_plot_array = np.nanmedian(
                    data_to_plot, axis=(1, 2)
                )
...
# plot the mean data
# Plot HCF
plot_annual_data_mean(
    results_anomaly=results_all_mean, var_name="HCF (%)"
)

# Plot IPR
plot_annual_data_mean(
    results_anomaly=results_all_mean, var_name="IPR (micron)"
)

# Plot CEH
plot_annual_data_mean(
    results_anomaly=results_all_mean, var_name="CEH (km)"
)


# plot the median data
# Plot HCF
plot_annual_data_median(
    results_anomaly=results_all_median, var_name="HCF (%)"
)

# Plot IPR
plot_annual_data_median(
    results_anomaly=results_all_median, var_name="IPR (micron)"
)

# Plot CEH
plot_annual_data_median(
    results_anomaly=results_all_median, var_name="CEH (km)"
)
