...
def process_each_year(data_merra2, cld_var_name: str = "Cldarea"):
    ...
    PC_data = {year: PC_years[year] for year in range(2016, 2021)}
    Cld_data = {year: Cld_years[year] for year in range(2016, 2021)}
    IWP_data = {year: IWP_years[year] for year in range(2016, 2021)}
    ...
    for year in range(2016, 2021):
        ...
        IWP_temp_filtered = IWP_temp_filtered.reshape(4, 28, 40, 360)
        AOD_temp_filtered = AOD_temp_filtered.reshape(4, 28, 40, 360)
        PC_temp_filtered = PC_temp_filtered.reshape(4, 28, 40, 360)
        Cld_temp_filtered = Cld_temp_filtered.reshape(4, 28, 40, 360)
...
def process_each_year_with_leave_one_out(
    ...
    for leave_out_year in years:
        ...
        process_years = [
            year for year in years if year != leave_out_year
        ]
        ...
        # Reshape filtered data
        IWP_temp_filtered = IWP_temp_filtered.reshape(
            4, 4, 28, 40, 360
        )
        AOD_temp_filtered = AOD_temp_filtered.reshape(
            4, 4, 28, 40, 360
        )
        PC_temp_filtered = PC_temp_filtered.reshape(4, 4, 28, 40, 360)
        Cld_temp_filtered = Cld_temp_filtered.reshape(
            4, 4, 28, 40, 360
        )
        ...
        save_filtered_data_as_nc(
            ...
            IWP_gap,
            PC_gap,
            save_str=f"leave_out_{leave_out_year}_filter_extreme_AOD_IWP_IPR_"
            + cld_var_name,
        )

        results[leave_out_year] = {
            "AOD_gap": AOD_gap,
            "IWP_gap": IWP_gap,
            "PC_gap": PC_gap,
            "Cld_data": Cld_match_PC_gap_IWP_AOD_constrain_mean,
...
results_HCF_leave_out = process_each_year_with_leave_one_out(
    data_merra2_2010_2020_new_lon, cld_var_name="Cldarea"
)

results_IPR_leave_out = process_each_year_with_leave_one_out(
    data_merra2_2010_2020_new_lon, cld_var_name="Cldicerad"
)

results_CEH_leave_out = process_each_year_with_leave_one_out(
    data_merra2_2010_2020_new_lon, cld_var_name="Cldeff_hgth"
)
...
# Plot IPR anomaly
plot_annual_data(results_anomaly=results_anomaly, var_name="IPR (micron)")
