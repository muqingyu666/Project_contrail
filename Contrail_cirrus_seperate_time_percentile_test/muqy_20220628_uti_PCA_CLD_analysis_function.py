...
def read_PC1_CERES_from_netcdf(PC_para_num, CERES_Cld_dataset_num):
    """
    Read the PC1 and CERES data from the netcdf file
    ...
    if PC_para_num == 0:
        ...
        data1 = xr.open_dataset(
            "/RAID01/data/PC_data/2010_2020_5_parameters_300hPa_PC1.nc"
        )
...
def read_PC1_only_from_netcdf(
    pc_data_name="1990_2020_4_parameters_300hPa_PC1.nc",
):
    """
...
# define the function to plot the spatial distribution of the data
def plot_spatial_var(
    ...
    Input:
        ...
        var_min: 变量最小值
        var_max: 变量最大值
        cmap_file: 自定义颜色表文件路径，默认为/RAID01/data/muqy/color/Var_color.txt
                    蓝色为低值，红色为高值，白色为缺测值
    """
...
class DividePCByDataVolume:
    def __init__(self, dataarray_main, n, start, end, gap):
    ...
    # calc the fomula of the PC gap -> var1, var2, coefficient, loop_num
    # var1 = (start * (loop_num - 1) / ((end - gap) - start))
    # coefficient = start / var1
    def give_loop_list_for_giving_gap(self):
        """
        Give the loop list for giving gap
    ...
    def main_gap(self):
        ...
        """
        # Flatten the data.
        self.dataarray_main = self.dataarray_main.flatten()
    ...
    # calculate the gap number for given main gap
    # etc, calculate the pc gap number for given pc value
    def gap_number_for_giving_main_gap(
        self,
    ):
        """Find the gap number for given main gap.
        :param n: number of main gap
        :return: gap number for given main gap
        """
        ...
        main_gap_min = main_gap_num[::2]
        main_gap_max = main_gap_num[1::2]

        n_min_max = int(len(main_gap_num) / 2)

        gap_num_min = np.empty((n_min_max))
        gap_num_max = np.empty((n_min_max))

        for i in range(n_min_max):
            gap_num_min[i] = (main_gap_min[i] / coef) - var1
            gap_num_max[i] = (main_gap_max[i] / coef) - var2

        return np.round(gap_num_min), np.round(gap_num_max)
    ...
    for lat, lon in np.ndindex((rows, cols)):
        for gap in range(start, end):
            ...
            if (
                ...
                # Cld_all_match_PC_gap_2020_sub_others[lat, lon] = value_2020 - value_others

                # Add checks for outliers, e.g., with a threshold of 500%
                if abs(percentage_difference) < 500:
                    Cld_all_match_PC_gap_2020_sub_others[
                        lat, lon
                    ] = percentage_difference
                else:
